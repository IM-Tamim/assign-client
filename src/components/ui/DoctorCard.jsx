    ...
    return (
        <div className="bg-base-100 rounded-2xl border border-base-300 overflow-hidden flex flex-col hover:shadow-lg hover:-translate-y-1 transition-all duration-300 group">
            ...
            <div className="relative h-52 overflow-hidden bg-base-200">
                ...
                />
                {/* Specialty badge */}
                <span className="absolute top-3 left-3 bg-error text-white text-xs font-semibold px-3 py-1 rounded-full z-10">
                    {doctor.specialty}
                </span>
                {/* Rating badge */}
                <span className="absolute top-3 right-3 bg-base-100/90 backdrop-blur-sm text-base-content text-xs font-bold px-2 py-1 rounded-full flex items-center gap-1 z-10">
            ...
            <div className="p-4 flex flex-col flex-1 gap-3">
                ...
                <div className="flex flex-col gap-1">
                    {doctor.availability?.slice(0, 2).map((slot, i) => (
                        <span key={i} className="text-xs text-base-content/60 flex items-center gap-1.5">
